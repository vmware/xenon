...
import static ${groupId}.XenonHost.startHost;

import java.net.URI;
import java.nio.file.Paths;
import java.util.logging.Level;

import ${groupId}.EmployeeService.Employee;
...
public class EmployeeServiceTest {
    ...
    public static void startHosts() throws Throwable {
        ...
        args.peerNodes = new String[numNodes];
        XenonHost[] hostList = new XenonHost[numNodes];
        for (int i = 0; i < numNodes; i++) {
            args.peerNodes[i] = "http://127.0.0.1:" + (8000 + i);
        }
        for (int i = 0; i < numNodes; i++) {
            args.id = "host" + i;   // human readable name instead of GUID
            args.sandbox = Paths.get(folder.getRoot().toString());
            args.port = 8000 + i;
            hostList[i] = startHost(new String[0], args);
        }

        TestNodeGroupManager nodeGroup = new TestNodeGroupManager();
        for (XenonHost host : hostList) {
    ...
    public void testPost() {
        ...
        TestUtils.checkServiceCount(h, Employee.class, numEmployees, 10000);
        h.log(Level.INFO, "Created %d records in %d ms", numEmployees, timer.end());

        deleteTestEmployees(numEmployees, prefix);
        TestUtils.checkServiceCount(h, Employee.class, 0, 10000);
    }
    ...
    public void testPut()  {
         ...
         */
        createTestEmployees(numEmployees, prefix);
        TestUtils.checkServiceCount(h, Employee.class, numEmployees, 20000);
     ...
     * requests have been requested.  Please see TestUtils.checkServiceCount(), which will wait for the index to show
     ...
     * recommend using TestUtils.checkServiceCount() to validate that all objects were created.
     *
