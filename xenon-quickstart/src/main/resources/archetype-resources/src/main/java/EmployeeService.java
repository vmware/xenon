...
public class EmployeeService extends StatefulService {
    ...
    // used in getDocumentTemplate() method.
    private static int VERSION_RETENTION_LIMIT = 5;     // keep 5 versions of employe changes
     ...
     * the proposed document body before we persist it.
    ...
    public void handlePatch(Operation patch) {
        ...
        if (patchBody.documentExpirationTimeMicros != state.documentExpirationTimeMicros) {
            state.documentExpirationTimeMicros = patchBody.documentExpirationTimeMicros;
        }
    ...
    // Could also override handleDelete() if you like.
     ...
     * @see ServiceDocumentDescription.PropertyIndexingOption for some Lucene index settings that can be set.
     */
    @Override
    public ServiceDocument getDocumentTemplate() {
        ...
        template.documentDescription.versionRetentionLimit = EmployeeService.VERSION_RETENTION_LIMIT;
