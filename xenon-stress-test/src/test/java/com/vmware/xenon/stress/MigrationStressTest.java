...
package com.vmware.xenon.stress;
...
public class MigrationStressTest {
    ...
    private static final int DEFAULT_SOURCE_HOST_PORT = 9000;
    private static final int DEFAULT_DEST_HOST_PORT = 9001;
     ...
     *   -Dexec.mainClass="com.vmware.xenon.stress.MigrationStressTest\$MigrationHost"  \
     *   -Dxenon.port=9000  \
     *   -Dxenon.sandbox=...
     *   -P performance &
     * </pre>
    ...
    public void setUp() throws Throwable {
        ...
        if (this.startHost) {
            ...
            VerificationHost sourceHost = prepareHost(0, DEFAULT_SOURCE_HOST_PORT, null);

            this.logger.log(Level.INFO, "Starting dest server");
            VerificationHost destHost = prepareHost(0, DEFAULT_DEST_HOST_PORT, null);
    ...
    public void simpleMigration() throws Throwable {
        ...
        if (TaskStage.FAILED == finalState.taskInfo.stage) {
            this.host.log(Level.WARNING, "Migration failed: %s", Utils.toJson(finalState.taskInfo.failure));
            return;
        }
    ...
    private void populateData(URI sourceHostUri) {
        ...
        for (long i = 0; i < total; i++) {
            ...
            if (ops.size() % batch == 0) {
                ...
                this.host.log("populating data: i=%,d, took=%d msec", i, batchEnd - batchStart);
                ops.clear();
            }
        }
