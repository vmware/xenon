...
/**
 * Simple stateful service used by {@link WordCountsService} and {@link LocalWordCountsService}.<br/>
 * The service is designed to store and retrieve documents with no additional logic unless the
 * document id is equal to {@link DocumentsService#ERROR_ID}. In that case the get operation
 * intentionally fails with RuntimeException so we can demonstrate error handling in the services.
 */
public class DocumentsService extends StatefulService {
    ...
    public static Service createFactory() {
        return FactoryService.create(DocumentsService.class);
    }
