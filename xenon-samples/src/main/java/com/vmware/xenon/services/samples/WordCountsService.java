 ...
 * @see LocalWordCountsService
 */
public class WordCountsService extends StatelessService {
    ...
    private DeferredResult<WordCountsResponse> countWordsInDocuments() {
        return this.sendBroadcastQuery()
                ...
                .thenApply(WordCountsService::aggregateResponses);
    }

    /**
     * Forward the query to {@link LocalWordCountsService} at each node.
     * @return
     */
    private DeferredResult<NodeGroupBroadcastResponse> sendBroadcastQuery() {
        URI localServiceURI = UriUtils.buildUri(this.getHost(), LocalWordCountsService.SELF_LINK);
    ...
    private List<WordCountsResponse> extractResponses(NodeGroupBroadcastResponse broadcastResponse) {
        return broadcastResponse.jsonResponses.values().stream()
                .map(json -> Utils.fromJson(json, WordCountsResponse.class))
