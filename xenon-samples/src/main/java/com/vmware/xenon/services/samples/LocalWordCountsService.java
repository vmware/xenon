 ...
 * </ol>
 */
public class LocalWordCountsService extends StatelessService {
    ...
    private DeferredResult<List<String>> fetchLocalDocumentLinks() {
        Operation query = Operation.createGet(this, DocumentsService.FACTORY_LINK);
     ...
     * @return
     */
    private DeferredResult<Document> fetchDocument(String documentLink) {
        Operation getDocument = Operation.createGet(this, documentLink);
        return this.sendWithDeferredResult(getDocument, Document.class);
    }
     ...
     * @return
     */
    private Map<String, Integer> countWords(Document document) {
        Map<String, Integer> result = new HashMap<>();
        if (document.contents == null) {
            return result;
        }

        String[] words = document.contents.split("[\\s.,!?:;(){}\\[\\]]+");
