...
public class TestSampleContinuousQueryObserverService {
    ...
    public void setupHost() throws Exception, Throwable {
        ...
        this.host.toggleServiceOptions(this.host.getDocumentIndexServiceUri(),
                ...
                null);
        this.sender = new TestRequestSender(host);
        host.startFactory(new SamplePreviousEchoService());
        host.startFactory(new SampleContinuousQueryObserverService());
    }
    ...
    public void testSampleContinuousQueryObserverService() throws Throwable {
        ...
        queryObserverInitialState = sender.sendAndWait(post, QueryObserverState.class);
        ...
        for (int i=0; i < serviceCount; i++) {
            ...
            state.message = "hello";
            post = Operation.createPost(UriUtils.buildUri(host.getUri(), SamplePreviousEchoService.FACTORY_LINK))
                    .setBody(state)
                    .setReferer(host.getUri());
            state = sender.sendAndWait(post, EchoServiceState.class);
        ...
        waitFor(this.timeout, () -> {
            Operation getQueryObserverState = Operation.createGet(
                    UriUtils.buildUri(host.getUri(), queryObserverSelfLink))
                    .setReferer(host.getUri());
            QueryObserverState updatedQueryObserverState =
                    sender.sendAndWait(getQueryObserverState, QueryObserverState.class);
            return (serviceCount == updatedQueryObserverState.notificationsCounter);
        });

        // update the state of all the SamplePreviousEchoService we created
        final EchoServiceState updatedMockState = new EchoServiceState();
        updatedMockState.message = "hello world";
        samplePreviousEchoServicesLinks.forEach(selfLink -> {
            Operation putOp = Operation.createPut(UriUtils.buildUri(host.getUri(), selfLink))
                    .setBody(updatedMockState)
                    .setReferer(host.getUri());
            sender.sendAndWait(putOp, EchoServiceState.class);
        });
        ...
        waitFor(this.timeout, () -> {
            ...
            QueryObserverState updatedQueryObserverState =
                    sender.sendAndWait(getQueryObserverState, QueryObserverState.class);
            return (serviceCount*2 == updatedQueryObserverState.notificationsCounter);
        });

        // delete all the services
        samplePreviousEchoServicesLinks.forEach(selfLink -> {
            Operation delete = Operation.createDelete(UriUtils.buildUri(host.getUri(), selfLink))
                    .setReferer(host.getUri());
        ...
        waitFor(this.timeout, () -> {
            ...
            QueryObserverState updatedQueryObserverState =
                    sender.sendAndWait(getQueryObserverState, QueryObserverState.class);
            return (serviceCount*3 == updatedQueryObserverState.notificationsCounter);
        });
    }
}
