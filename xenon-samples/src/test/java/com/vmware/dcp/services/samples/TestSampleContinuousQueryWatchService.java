...
public class TestSampleContinuousQueryWatchService {
    ...
    public void setupHost() throws Throwable {
        ...
        this.host.waitForServiceAvailable(SampleContinuousQueryWatchService.FACTORY_LINK);
    }
    ...
    public void testSampleWatchService() throws Throwable {
        ...
        Operation post = Operation.createPost(this.host, SampleContinuousQueryWatchService.FACTORY_LINK)
        ...
        waitFor(this.timeout, () -> {
            Map<String, ServiceStat> indexStats = this.host.getServiceStats(this.host.getDocumentIndexServiceUri());
        ...
        for (int i = 0; i < this.serviceCount; i++) {
            opList.add(Operation.createPost(this.host, SamplePreviousEchoService.FACTORY_LINK).setBody(state));
        }

        samplePreviousEchoServicesLinks = this.sender.sendAndWait(
        ...
        waitFor(this.timeout, () -> {
            Operation getQueryWatchState = Operation.createGet(this.host, queryWatchSelfLink);
            ...
            this.host.log("notification count: %d", updatedWatchState.notificationsCounter);
        ...
        this.host.testStart(samplePreviousEchoServicesLinks.size());
        samplePreviousEchoServicesLinks.forEach(selfLink -> {
            Operation.createPut(this.host, selfLink)
                    .setBody(updatedState)
                    .setCompletion(this.host.getCompletion()).sendWith(this.sender);
        });
        this.host.testWait();
        ...
        waitFor(this.timeout, () -> {
            ...
            SampleContinuousQueryWatchService.State updatedQueryWatchState =
                    this.sender.sendAndWait(getQueryWatchState, SampleContinuousQueryWatchService.State.class);
            this.host.log("notification count: %d", updatedQueryWatchState.notificationsCounter);
        ...
        this.host.testStart(samplePreviousEchoServicesLinks.size());
        samplePreviousEchoServicesLinks.forEach(selfLink -> {
            Operation.createDelete(this.host, selfLink).setCompletion(this.host.getCompletion()).sendWith(this.sender);
        });
        this.host.testWait();
