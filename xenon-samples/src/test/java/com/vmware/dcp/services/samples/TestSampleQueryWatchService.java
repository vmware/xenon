...
import com.vmware.xenon.services.samples.SampleQueryWatchService;
...
public class TestSampleQueryWatchService {
    ...
    public void testSampleWatchService() throws Throwable {
        // create a new SampleQueryWatchService with
        // notification counter set to 0
        SampleQueryWatchService.State sampleQueryWatchState = new SampleQueryWatchService.State();
        sampleQueryWatchState.notificationsCounter = 0;
        Operation post = Operation.createPost(UriUtils.buildUri(this.host.getUri(),
                SampleQueryWatchService.FACTORY_LINK))
                .setBody(sampleQueryWatchState)
                .setReferer(this.host.getUri());
        sampleQueryWatchState = this.sender.sendAndWait(post, SampleQueryWatchService.State.class);
        // remember the link to the SampleQueryWatchService we just created
        ...
        // get the SampleQueryWatchService.State and make sure that it was notified
        ...
        waitFor(this.timeout, () -> {
            Operation getQueryWatchState = Operation.createGet(
                    ...
                    .setReferer(this.host.getUri());
            SampleQueryWatchService.State updatedWatchState =
                    this.sender.sendAndWait(getQueryWatchState, SampleQueryWatchService.State.class);
        ...
        waitFor(this.timeout, () -> {
            Operation getQueryWatchState = Operation.createGet(
                    UriUtils.buildUri(this.host.getUri(), queryWatchSelfLink))
                    .setReferer(this.host.getUri());
            SampleQueryWatchService.State updatedQueryWatchState =
                    this.sender.sendAndWait(getQueryWatchState, SampleQueryWatchService.State.class);
            this.host.log("notification count: %d",
