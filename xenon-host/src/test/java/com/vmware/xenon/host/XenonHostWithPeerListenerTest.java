...
import com.vmware.xenon.common.http.netty.Netty2WaySslAuthTest;
...
public class XenonHostWithPeerListenerTest {
    ...
    private void assertCanGetNodeGroup(ServiceClient client, URI u)
        ...
        Operation.createGet(u)
                //   .forceRemote()
                .setReferer(u)
                .setCompletion(complete(fut))
                .sendWith(client);
        Operation op = fut.get();
        assertEquals(Operation.STATUS_CODE_OK, op.getStatusCode());
    }

    private CompletionHandler complete(CompletableFuture<Operation> fut) {
        return (o, e) -> {
            if (e == null) {
                fut.complete(o);
            } else {
