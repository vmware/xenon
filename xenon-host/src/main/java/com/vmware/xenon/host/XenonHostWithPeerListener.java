 ...
 * If --nodeGroupPublicUri is node configured it behaves like a regulat XenonHost.
 * If --nodeGroupPublicUri value starts with https: then a secure listener is started. The certificate of the
 * secure listener default to the one used for client-server communication unless
...
public class XenonHostWithPeerListener extends ServiceHost {
    ...
    private void validatePeerArgs() {
        ...
        if (uri.getPort() < 0) {
            throw new IllegalArgumentException("the port of --nodeGroupPublicUri must be set to a valid port number");
        }

        if (uri.getPort() == this.hostArgs.port) {
