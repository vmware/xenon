 ...
 * verbs for a {@link GatewayService}
 */
public class PathService extends StatefulService {

    public static final String FACTORY_LINK = GatewayUriPaths.PATHS;

    public static FactoryService createFactory() {
        return FactoryService.create(PathService.class);
    }
    ...
    public static class State extends ServiceDocument {
        ...
        /**
         * Set of Http Verbs allowed on the
        ...
        /**
         * Instructs the gateway service to skip verb
         * checking and allow all requests for this
         * Uri path.
         */
        @UsageOption(option = PropertyUsageOption.AUTO_MERGE_IF_NOT_NULL)
        public Boolean allowAllVerbs = true;
    }
    ...
    private State validateStartState(Operation start) {
        ...
        boolean allowAll = (state.allowAllVerbs != null && state.allowAllVerbs == true);
        if (!allowAll && (state.verbs == null || state.verbs.isEmpty())) {
            start.fail(new IllegalStateException("verbs are missing"));
            return null;
        }
    ...
    public static final String createSelfLinkFromState(State state) {
        ...
        return FACTORY_LINK + UriUtils.URI_PATH_CHAR + state.gatewayId + '-' + path;
    }
