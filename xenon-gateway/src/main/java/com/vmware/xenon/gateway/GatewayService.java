...
public class GatewayService extends StatelessService {
    ...
    public void handleRequest(Operation op) {
        ...
        final long startTimeMicros = Utils.getNowMicrosUtc();
    ...
    // Sample: POST /core/examples 200 11.00ms
    private void logRequest(Operation op, long startTimeMicros, int statusCode, long contentLength) {
        logInfo("%s %s %d %dB %.2fms", op.getAction(), op.getUri().getPath(),
                statusCode, contentLength, (Utils.getNowMicrosUtc() - startTimeMicros) / 1000.0);
    }
}
