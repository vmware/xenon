...
import com.vmware.xenon.services.common.IdGeneratorService;
import com.vmware.xenon.services.common.IdRangeService;
...
public class TestIdGeneratorService extends BasicReusableHostTestCase {
    ...
    private void setUpMultiNode() throws Throwable {
        ...
        for (VerificationHost h : this.host.getInProcessHostMap().values()) {
            ...
            this.host.log(Level.INFO, "%s", UriUtils.buildUri(h.getUri(), IdGeneratorService.SELF_LINK));
            this.host.log(Level.INFO, "%s", UriUtils.buildUri(h.getUri(), IdRangeService.FACTORY_LINK));
        }

        this.host.getPeerHost().waitForServiceAvailable(IdRangeService.FACTORY_LINK);
        for (VerificationHost h : this.host.getInProcessHostMap().values()) {
            h.startService(new IdGeneratorService());
            break;
        }
        ...
        URI factoryUri = UriUtils.buildUri(
                this.host.getPeerHost(), IdRangeService.FACTORY_LINK);
