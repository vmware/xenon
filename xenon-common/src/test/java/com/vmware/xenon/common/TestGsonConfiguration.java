...
public class TestGsonConfiguration {
    ...
    public void testJsonRedaction() {
        ...
        JsonElement expected = readJson("{ \"value\": { "
                ...
                + "\"b\": \"barney\" "
                + "}, "
                + "\"sensitiveUsageOption\": \"sensitive information\", "
                + "\"sensitivePropertyOptions\": \"sensitive properties\" "
                + BORING_JSON_DOC_BITS
                + "}");
        assertEquals(expected, readJson(compact));

        String compactRedacted = Utils.toJsonRedacted(doc);
    ...
    public void testPrettyPrintingRedaction() {
        ...
        String compactRedacted = Utils.toJsonRedacted(doc);
        String prettyRedacted = Utils.toJsonHtmlRedacted(doc);
