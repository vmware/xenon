...
public class TestFutures extends BasicReusableHostTestCase {
    ...
    public void asBiFunction() {
        ...
        BiFunction<Operation, Throwable, Void> func = Futures.asFunc(h);
    ...
    public void completeExce() throws InterruptedException {
        CompletableFuture<Operation> f = Operation
                .createGet(UriUtils.buildUri(this.host, "/hello"))
                .setReferer(this.host.getReferer())
                .sendFuture(this.host);
    ...
    public void seq() throws InterruptedException, ExecutionException {
        ...
        CompletableFuture<List<Operation>> join = Futures.join(futs);
        List<Operation> allServices = join.get();

        for (Operation service : allServices) {
            System.out.println(service.getBody(AuthCredentialsServiceState.class).documentSelfLink);
        }
        ...
        // chain 10
        CompletableFuture<Operation> f = getFirst.sendFuture(this.host);
        for (int i = 1; i < 10; i++) {
    ...
    private CompletableFuture<Operation> getNextService(Operation o) {
        ...
        return Operation.createGet(this.host,
