...
class SynchRetryExampleService extends StatefulService {
    ...
    public void handleRequest(Operation op) {
        if (op.getUri().getPath().endsWith("fail")) {
...
public class TestSynchronizationTaskService extends BasicTestCase {
    ...
    public void serviceResynchOnFailure() throws Throwable {
        ...
        createExampleServices(sender, this.host, this.serviceCount / 2, "pass");
        createExampleServices(sender, this.host, this.serviceCount / 2, "fail");
        ...
        assertEquals(this.serviceCount, result.synchCompletionCount);
        ...
        waitForSynchRetries(result, SynchronizationTaskService.STAT_NAME_CHILD_SYNCH_FAILURE_COUNT,
                (synchRetryCount) -> synchRetryCount.latestValue == this.serviceCount / 2);
    ...
    private List<URI> createExampleServices(
        ...
        List<ServiceDocument> result =  sender.sendAndWait(ops, ServiceDocument.class);
