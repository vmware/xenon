...
public class TestDeferredResult {
    ...
    private void runAfterAndCountdown(long millis, Runnable action, CountDownLatch latch) {
        runAfter(millis, () -> {
            action.run();
            latch.countDown();
        });
    ...
    private void runAfter(CountDownLatch latch, Runnable action) {
        runAfter(latch, 0, action);
    }

    private void runAfter(CountDownLatch latch, long additionalDelay, Runnable action) {
        new Thread(() -> {
            try {
                latch.await();
            } catch (InterruptedException e) {
    ...
    private void verifyApplyToEither(long wait, boolean originalFirst) throws Throwable {
        ...
        if (originalFirst) {
            runAfterAndCountdown(wait, () -> original.complete(originalValue), latch);
            runAfter(latch, () -> other.complete(otherValue));
        } else {
            runAfter(latch, () -> original.complete(originalValue));
            runAfterAndCountdown(wait, () -> other.complete(otherValue), latch);
        }
        ctx.await();
        Assert.assertEquals(1, invocations.get());
    ...
    private void verifyAcceptEither(long wait, boolean originalFirst) throws Throwable {
        ...
        if (originalFirst) {
            runAfterAndCountdown(wait, () -> original.complete(originalValue), latch);
            runAfter(latch, wait, () -> other.complete(otherValue));
        } else {
            runAfter(latch, wait, () -> original.complete(originalValue));
            runAfterAndCountdown(wait, () -> other.complete(otherValue), latch);
        }
        ctx.await();
        Assert.assertTrue(System.currentTimeMillis() - start < wait * 2);
        Assert.assertEquals(1, invocations.get());
    }
    ...
    public void testAnyOf() throws Throwable {
        ...
        IntStream.range(0, count).forEach(i -> {
            if (i == fastest) {
                runAfterAndCountdown(1, () -> deferredResults.get(i).complete(i), latch);
            } else {
                runAfter(latch, () -> deferredResults.get(i).complete(i));
            }
        });
        ctx.await();
