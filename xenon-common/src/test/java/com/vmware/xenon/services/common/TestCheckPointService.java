...
public class TestCheckPointService extends BasicTestCase {
    ...
    private static final String EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK = "/synch-check-point/example";
    ...
    public void setUp(int nodeCount) throws Throwable {
        ...
        if (this.host.getInProcessHostMap().isEmpty()) {
            ...
            for (VerificationHost host : this.host.getInProcessHostMap().values()) {
                ...
                Operation post = Operation.createPost(UriUtils.buildUri(host, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                        .setBody(s);
    ...
    public void testCheckPointServiceRestart() throws Throwable {
        ...
        h0.start();
        Operation startPost = Operation.createPost(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                .setBody(null);
        h0.startService(startPost, new CheckPointService());
        h0.waitForServiceAvailable(EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK);
        h0.waitForServiceAvailable(ExampleService.FACTORY_LINK);
        this.host.addPeerNode(h0);
        this.host.waitForNodeGroupIsAvailableConvergence();

        Operation get = Operation.createGet(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK));
    ...
    public void testCheckPointServiceUpdate() throws Throwable {
        ...
        Operation patch = Operation.createPatch(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                .setBody(state);
        ...
        patch = Operation.createPatch(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                .setBody(state);
        this.host.getTestRequestSender().sendAndWait(patch);
        get = Operation.createGet(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK));
    ...
    public void testCheckPointServiceMultiNode() throws Throwable {
        ...
        long oldCheckPoint = this.checkPoint++;
        state.checkPoint = this.checkPoint;
        // update local check point of h0, no propagation
        Operation patch = Operation.createPatch(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                .setBody(state);
        this.host.getTestRequestSender().sendAndWait(patch);

        for (ServiceHost h : this.host.getInProcessHostMap().values()) {
            Operation op = Operation.createGet(UriUtils.buildUri(h, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK));
            CheckPointService.CheckPointState s =
        ...
        // update checkpoint within rest of nodes
        oldCheckPoint = this.checkPoint++;
        for (ServiceHost h : this.host.getInProcessHostMap().values()) {
            CheckPointService.CheckPointState s = new CheckPointService.CheckPointState();
            s.checkPoint = this.checkPoint;
            Operation op = Operation.createPatch(UriUtils.buildUri(h, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                    .setBody(s);
            this.host.send(op);
        }
        // restart h0, load local checkpoint
        h0.start();
        Operation startPost = Operation.createPost(UriUtils.buildUri(h0, EXAMPLE_SYNCH_CHECKPOINT_SELF_LINK))
                .setBody(null);
        h0.startService(startPost, new CheckPointService());
