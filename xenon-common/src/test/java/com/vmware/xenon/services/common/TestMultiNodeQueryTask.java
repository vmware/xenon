...
public class TestMultiNodeQueryTask extends BasicTestCase {
    ...
    public void setUp() throws Throwable {
        ...
        for (VerificationHost h : host.getInProcessHostMap().values()) {
            h.startFactory(new ExampleFooService());
            h.startFactory(new ExampleBarService());
        }
        host.waitForReplicatedFactoryServiceAvailable(UriUtils.buildUri(this.peerUri, ExampleFooService.FACTORY_LINK));
        host.waitForReplicatedFactoryServiceAvailable(UriUtils.buildUri(this.peerUri, ExampleBarService.FACTORY_LINK));
    }
