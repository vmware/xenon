...
public class TestCheckPointTaskService extends BasicTestCase {
    ...
    private final QueryTask.Query customExampleQuery = QueryTask.Query.Builder.create()
            .addKindFieldClause(ExampleServiceState.class)
            .addFieldClause(ServiceDocument.FIELD_NAME_SELF_LINK,
            ...
            .build();
    private boolean checkPointSync = false;
    ...
    public static class CustomExampleFactoryService extends FactoryService {
        ...
        @Override
        public void handleNodeGroupMaintenance(Operation mainOp) {
            // CustomFactory does not sync child service
            setAvailable(true);
            mainOp.complete();
        }
    }
    ...
    public void verifyScheduleCheckPointTask() throws Throwable {
        ...
        for (VerificationHost host : this.host.getInProcessHostMap().values()) {
            startTaskScheduleService(host, taskTemplate, CheckPointTaskService.FACTORY_LINK, this.schedulePeriod);
        }
        List<ExampleServiceState> exampleStates =
                this.host.createExampleServices(this.host.getPeerHost(), this.serviceCount, null, ExampleService.FACTORY_LINK);
        Map<String, ExampleServiceState> exampleStatesMap =
        ...
        startTaskScheduleService(h0, taskTemplate, CheckPointTaskService.FACTORY_LINK, this.schedulePeriod);
        this.host.joinNodesAndVerifyConvergence(this.nodeCount);
    ...
    public void verifyNodeLeftThenJoinSynchOnDemand() throws Throwable {
        this.checkPointSync = true;
        setUp(3);
        this.host.setNodeGroupQuorum(this.nodeCount - 1);
        this.host.waitForNodeGroupConvergence();

        // start task scheduler
        CheckPointTaskService.State taskTemplate = validateTaskState(this.customExampleQuery);
        for (VerificationHost host : this.host.getInProcessHostMap().values()) {
            startTaskScheduleService(host, taskTemplate, CheckPointTaskService.FACTORY_LINK, this.schedulePeriod);
        }
        List<ExampleServiceState> exampleStates =
    ...
    public void NodeSequentialJoinSynchOnDemand() throws Throwable {
        ...
        for (int i = 1; i < this.nodeCount; i++) {
            VerificationHost h = setUpLocalPeerHost();
            startTaskScheduleService(h, taskTemplate, CheckPointTaskService.FACTORY_LINK, this.schedulePeriod);
    ...
    private CheckPointTaskService.State validateTaskState(QueryTask.Query query) {
        ...
        s.onDemandSync = this.checkPointSync;
        return s;
    }
    ...
    public void startTaskScheduleService(VerificationHost host, TaskService.TaskServiceState taskTemplate, String taskFactoryLink, long schedulePeriod) {
        TaskScheduleService tss = new TaskScheduleService(taskTemplate, taskFactoryLink, schedulePeriod);
