...
public class TestNodeGroupService {
    ...
    @Test
    public void testQueryDocumentByVersionAndOwner() throws Throwable {
        ...
        testQueryDocumentByVersionAndOwnerHelper(versions, serviceCount, exampleStates, true);
        testQueryDocumentByVersionAndOwnerHelper(versions, serviceCount, exampleStates, false);
    }

    private void testQueryDocumentByVersionAndOwnerHelper(int versions, int serviceCount,
            Map<URI, ExampleServiceState> exampleStates, boolean valid) throws Throwable {

        this.host.testStart(serviceCount * versions);

        // Verify query by version and host works
        for (ExampleServiceState serviceDocument : exampleStates.values()) {
            for (long version = 0; version < versions; version++) {
                ...
                final String owner = valid ? serviceDocument.documentOwner : "invalid-host";
                QueryTask task = NodeSelectorSynchronizationService.buildVersionAndOwnerQueryTask(
                        this.host, finalVersion, owner, serviceDocument.documentSelfLink);
                        ...
                        .setCompletion((o, f) -> {
                            if (f != null) {
                                ...
                                if (valid) {
                                    ...
                                    assertEquals(result.results.documents.size(), 0);
                                }
                                this.host.completeIteration();
                            }
                        });
