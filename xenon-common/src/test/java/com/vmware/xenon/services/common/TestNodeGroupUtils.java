...
import com.vmware.xenon.services.common.BroadcastResponse.BroadcastSingleResponse;

public class TestNodeGroupUtils {

    @Test
    public void toBroadcastResponse() {
        ...
        BroadcastResponse result = NodeGroupUtils.toBroadcastResponse(seed);
        ...
        Set<BroadcastSingleResponse> successes = result.successResponses;
        assertEquals(3, successes.size());
        List<BroadcastSingleResponse> sorted = successes.stream()
                .sorted((left, right) -> left.hostId.compareTo(right.hostId))
                .collect(toList());

        // success1
        BroadcastSingleResponse singleResponse = sorted.get(0);
        ...
        // failure
        Set<BroadcastSingleResponse> failures = result.failureResponses;
        assertEquals(1, failures.size());
        BroadcastSingleResponse failure = failures.iterator().next();
