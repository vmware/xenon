...
public class TestNodeGroupDocumentOwnerChange extends BasicTestCase {
    ...
    public void documentOwnerChange() throws Throwable {
        ...
        factoryOwner.waitForReplicatedFactoryServiceAvailable(UriUtils.buildUri(factoryOwner,
                NodeGroupChangeCheckService.FACTORY_LINK));
        ...
        factoryOwner
                .waitFor(
                        "doc owner not set",
                        () -> {
                            docIdsWithoutDocumentOwner.clear();
                            List<Operation> getToCheckOwnerDocument = new ArrayList<>();
                            for (int i = 0; i < numOfDocs; i++) {
                                String selfLinkPath = NodeGroupChangeCheckService.FACTORY_LINK
                                        + "/"
                                        + i + "?CHECK_OWNER";
