...
public class TestCheckpointService extends BasicTestCase {
    ...
    public void setUp(int nodeCount) throws Throwable {
        ...
        if (this.host.getInProcessHostMap().isEmpty()) {
            ...
            this.host.waitForReplicatedFactoryServiceAvailable(
                    UriUtils.buildUri(this.host.getPeerHost(), CheckpointExampleService.FACTORY_LINK));
        }
    }
    ...
    private void verifySynchTask(long expectedMinMissTime, long expectedMaxHitTime, long expectedSynchCompletionCount) {
        this.host.waitFor("synch task finish timeout", () -> {
            ...
            long min = Long.min(expectedMinMissTime - 1, expectedMaxHitTime);
            long max = Long.max(expectedMinMissTime - 1, expectedMaxHitTime);
            if (newState.checkpoint < min || newState.checkpoint > max) {
                this.host.log(Level.INFO, "actual checkpoint %d, expected checkpoint range [%d, %d]",
                        newState.checkpoint, min, max);
                return false;
            }
