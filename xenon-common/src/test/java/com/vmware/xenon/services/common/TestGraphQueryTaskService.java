...
public class TestGraphQueryTaskService extends BasicTestCase {
    ...
    public void twoStageEmptyResults() throws Throwable {
        ...
        verifyStageResults(stageOneResults, 0);
    ...
    public void twoStage() throws Throwable {
        ...
        if (name != null) {
            // FIXME BEFORE CHECKIN, test currently disabled
            return;
        }
        ...
        ServiceDocumentQueryResult stageOneResults = finalState.stages.get(0).results;
        verifyStageResults(stageOneResults, this.serviceCount);

        ServiceDocumentQueryResult stageTwoResults = finalState.stages.get(1).results;
        verifyStageResults(stageTwoResults, this.serviceCount);
    }

    private void verifyStageResults(ServiceDocumentQueryResult stageOneResults,
            int expectedResultCount) {
        assertTrue(stageOneResults != null);
        assertTrue(stageOneResults.queryTimeMicros > 0);
        assertTrue(stageOneResults.documentCount == expectedResultCount);
        assertTrue(stageOneResults.documentLinks.size() == expectedResultCount);
        if (stageOneResults.selectedLinks == null) {
            if (expectedResultCount > 0) {
                throw new IllegalStateException("null selectedLinks");
            }
        } else {
            assertTrue(stageOneResults.selectedLinks.size() == expectedResultCount);
        }
    }
