...
public class AuthorizationFilter implements Filter {
    ...
    public FilterReturnCode processRequest(Operation op, OperationProcessingContext context) {
        if (!context.host.isAuthorizationEnabled() || context.host.getAuthorizationService() == null) {
    ...
    private void checkAndPopulateAuthzContext(Operation op, OperationProcessingContext context) {
        Service authzService = context.host.getAuthorizationService();
        ...
        context.host.queueOrScheduleRequest(authzService, op);
    }
    ...
    private void getAuthorizationContext(Operation op, OperationProcessingContext context, Consumer<AuthorizationContext> authorizationContextHandler) {
        ...
        AuthorizationContext ctx = context.host.getAuthorizationContext(null, token);
        if (ctx != null) {
                            ...
                            if (ex != null) {
                                ...
                                if (shouldRetry) {
                                    ...
                                    if (err.getErrorCode()
                                        ...
                                        context.opProcessingChain.resumedRequestFailed(parentOp, context, ex);
