...
public class OperationProcessingChain {
    ...
    public void resumeProcessingRequest(Operation op, OperationProcessingContext context,
        ...
        if (filterReturnCode == FilterReturnCode.RESUME_PROCESSING) {
            // resume operation processing starting from the current filter -
            // better run through the thread pool to avoid stack overflow
            context.host.run(() -> {
                FilterReturnCode rc = FilterReturnCode.CONTINUE_PROCESSING;
                rc = processRequest(op, context, context.currentFilterPosition);
                if (rc == FilterReturnCode.CONTINUE_PROCESSING) {
        ...
        // continue processing starting from the next filter
        FilterReturnCode rc = FilterReturnCode.CONTINUE_PROCESSING;
        int nextFilterStartIndex = context.currentFilterPosition + 1;
