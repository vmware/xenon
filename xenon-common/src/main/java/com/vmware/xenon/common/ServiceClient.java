...
public interface ServiceClient extends ServiceRequestSender {
    ...
    int MAX_BINARY_SERIALIZED_BODY_LIMIT = (int) XenonConfiguration.integer(
            ServiceClient.class,
            "MAX_BINARY_SERIALIZED_BODY_LIMIT",
            1024 * 1024
    );

    int DEFAULT_CONNECTION_LIMIT_PER_HOST = (int) XenonConfiguration.integer(
            ServiceClient.class,
            "DEFAULT_CONNECTIONS_PER_HOST", 128
    );

    int DEFAULT_CONNECTION_LIMIT_PER_TAG = (int) XenonConfiguration.integer(
            ServiceClient.class,
            "DEFAULT_CONNECTION_LIMIT_PER_TAG", 4);

    int REQUEST_PAYLOAD_SIZE_LIMIT = (int) XenonConfiguration.integer(
            ServiceClient.class,
            "REQUEST_PAYLOAD_SIZE_LIMIT",
            1024 * 1024 * 64
    );

    int DEFAULT_PENDING_REQUEST_QUEUE_LIMIT = (int) XenonConfiguration.integer(
            ServiceClient.class,
