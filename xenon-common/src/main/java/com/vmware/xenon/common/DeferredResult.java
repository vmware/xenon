...
public final class DeferredResult<T> {
     ...
     * @throws NotCompletedException if the DeferredResult is not yet completed.
     * @return The value of this DeferredResult if it is already completed.
     */
    public T getNow() throws NotCompletedException {
        if (this.completableFuture.isDone()) {
            return this.completableFuture.join();
        }
        throw new NotCompletedException("The result is not done!");
    }
