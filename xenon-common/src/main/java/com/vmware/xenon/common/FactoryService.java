...
public abstract class FactoryService extends StatelessService {
    ...
    // Reset synchronization retry count usage in delay calculation after this value.
    public static final int DEFAULT_SYNCH_RETRY_LIMIT = 20;
    ...
    private void scheduleSynchronizationRetry(Operation parentOp) {
        ...
        if (stat != null && stat.latestValue  > 0) {
            if (stat.latestValue >= DEFAULT_SYNCH_RETRY_LIMIT) {
                logSevere("Synchronization task failed after %d tries", stat.latestValue);
                return;
            }
        }
