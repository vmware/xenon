...
public abstract class FactoryService extends StatelessService {
    ...
    private void startFactorySynchronizationTask(Operation parentOp, Long membershipUpdateTimeMicros) {
        ...
        task.documentSelfLink = createSynchTaskSelfLink(this.getSelfLink());
    ...
    private String createSynchTaskSelfLink(String factoryLink) {
        String l = factoryLink.replace("/", "-");
        int s = 0;
        int e = l.length() - 1;
        if (l.charAt(s) == '-') {
            s = 1;
        ...
        return (s == 0 && e == l.length() - 1) ? l : l.substring(s, e);
    }
