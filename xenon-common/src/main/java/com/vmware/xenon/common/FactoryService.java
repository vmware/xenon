...
public abstract class FactoryService extends StatelessService {
    ...
    Service createChildService() throws Throwable {
        Service childService;
    ...
    public void handleNodeGroupMaintenance(Operation maintOp) {
        ...
        selectOwnerOp.setCompletion((o, e) -> {
            ...
            if (rsp.isLocalHostOwner == false) {
                ...
                cancelSynchTask(maintOp, rsp.membershipUpdateTimeMicros);
                return;
            }

            if (rsp.availableNodeCount > 1) {
                verifySynchronizationOwnership(maintOp, rsp);
                return;
            }
    ...
    private void startFactorySynchronizationTask(Operation parentOp, Long membershipUpdateTimeMicros) {
        ...
        task.childOptions = this.childOptions;
        task.documentExpirationTimeMicros = Utils.getNowMicrosUtc() + TimeUnit.SECONDS.toMicros(60);
    ...
    private void cancelSynchTask(Operation maintOp, long membershipUpdateTimeMicros) {
        SynchronizationTaskService.State task = new SynchronizationTaskService.State();
        task.childOptions = this.childOptions;
        task.documentExpirationTimeMicros = Utils.getNowMicrosUtc() + TimeUnit.SECONDS.toMicros(60);
        task.documentSelfLink = this.getSelfLink();
    ...
    private void verifySynchronizationOwnership(Operation maintOp, SelectOwnerResponse ownerResponse) {
        ...
        Operation broadcastSelectOp = Operation
                ...
                .setCompletion((op, t) -> {
                    ...
                    if (consensusCount < (ownerResponse.availableNodeCount / 2) + 1) {
                        logWarning("Majority nodes do not consider the current node owner for this factory");
                        cancelSynchTask(maintOp, ownerResponse.membershipUpdateTimeMicros);
                        return;
                    }

                    logInfo("Elected owner on %s, starting synch (%d)",
                            getHost().getId(), ownerResponse.availableNodeCount);
