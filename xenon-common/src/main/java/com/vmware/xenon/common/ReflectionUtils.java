        ...
        try {
            ...
            if (currentObj != null) {
                if (currentObj instanceof Collection) {
                    ...
                    Class<?> clazz = currentObj.getClass().getComponentType();
                    int sourceArrayLen = java.lang.reflect.Array.getLength(currentObj);
                    int patchArrayLen = java.lang.reflect.Array.getLength(value);
                    Object resultArrayObj = java.lang.reflect.Array.newInstance(clazz,
                            sourceArrayLen + patchArrayLen);
                    int destIndex = 0;
                    for (int i = 0; i < sourceArrayLen; i++, destIndex++) {
                        Array.set(resultArrayObj, destIndex, Array.get(currentObj, i));
                    }
                    for (int i = 0; i < sourceArrayLen; i++, destIndex++) {
                        Array.set(resultArrayObj, destIndex, Array.get(value, i));
                    }
                    pd.accessor.set(instance, resultArrayObj);
                } else {
