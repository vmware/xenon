...
public class NettyHttpClientRequestHandler extends SimpleChannelInboundHandler<Object> {
    ...
    /**
     * When this is set to true, include "x-request-id" header in request log
     */
    private static final boolean INCLUDE_REQUEST_ID_IN_REQUEST_LOG = XenonConfiguration.bool(
            NettyHttpClientRequestHandler.class,
            "includeRequestIdInRequestLog",
            false
    );
    ...
    private void writeResponse(ChannelHandlerContext ctx, Operation request,
        ...
        if (this.host.isRequestLoggingEnabled()) {
            ...
            if (!avoidLogging) {
                ...
                if (INCLUDE_REQUEST_ID_IN_REQUEST_LOG) {
