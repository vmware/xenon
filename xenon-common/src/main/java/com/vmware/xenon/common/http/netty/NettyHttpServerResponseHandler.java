...
public class NettyHttpServerResponseHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        ...
        this.logger.warning(String.format("Channel exception, failing request %d, uri %s, channel %s, cause: %s",
                request.getId(), request.getUri().toString(), ctx.channel().id().asLongText(), Utils.toString(cause)));
