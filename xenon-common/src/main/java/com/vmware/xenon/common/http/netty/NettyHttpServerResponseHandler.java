...
public class NettyHttpServerResponseHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    private Operation findOperation(ChannelHandlerContext ctx, FullHttpResponse response) {
        ...
        if (ctx.channel().hasAttr(NettyChannelContext.HTTP2_KEY)) {
            ...
            if (streamId == null) {
                logger.warning("HTTP/2 message has no stream ID: ignoring.");
                return null;
            }
            NettyChannelContext channelContext = ctx.channel().attr(NettyChannelContext.CHANNEL_CONTEXT_KEY).get();
            if (channelContext == null) {
                logger.warning("HTTP/2 channel is missing associated channel context: ignoring response on stream " + streamId);
                return null;
            }
            request = channelContext.getOperationForStream(streamId);
            if (request == null) {
                logger.warning("Can't find operation for stream " + streamId);
                return null;
            }
            ...
            if (request == null) {
                logger.warning("Can't find operation for channel");
                return null;
            }
        }
        return request;
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        logger.warning(Utils.toString(cause));
        ...
        if (request == null) {
            ...
            // find all the requests and fail all of them. That's slightly risy because
            // we don't understand why we failed, and we may get responses for them later.
            Logger.getAnonymousLogger().info(
                    "Channel exception, no HTTP/1.1 request to fail" + cause.getMessage());
            return;
        }
