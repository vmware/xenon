 ...
 *
 *   AuthorizationUtil.Builder.create()
 *         .setHost(this)
 ...
 *         .start();
 *
 * As a note on the limitiations: this user doesn't have access to modify their
...
public class AuthorizationUtil {
    ...
    private enum UserCreationStep {
        QUERY_USER, MAKE_USER, MAKE_CREDENTIALS, MAKE_USER_GROUP, MAKE_RESOURCE_GROUP, MAKE_ROLE, SUCCESS
    }
    ...
    /**
     * Class to build the AuthorizationUtil class and start the workflow for making the user
     */
    public static class Builder {
        private final AuthorizationUtil authUtil;

        private Builder() {
            this.authUtil = new AuthorizationUtil();
        }
        ...
        public Builder setUserEmail(String userEmail) {
            this.authUtil.userEmail = userEmail;
            return this;
        }

        public Builder setUserPassword(String userPassword) {
            this.authUtil.userPassword = userPassword;
            return this;
        }

        public Builder setIsAdmin(boolean isAdmin) {
            this.authUtil.isAdmin = isAdmin;
            return this;
        }

        public Builder setDocumentKind(String documentKind) {
            this.authUtil.documentKind = documentKind;
            return this;
        }

        public Builder setHost(ServiceHost host) {
            this.authUtil.host = host;
            this.authUtil.referer = UriUtils.buildUri(host, "");
            return this;
        }

        public AuthorizationUtil start() {
            validate();
            this.authUtil.currentStep = UserCreationStep.QUERY_USER;
            this.authUtil.setupUser();
            return this.authUtil;
        }

        private void validate() {
            if (this.authUtil.userEmail == null) {
                throw new IllegalStateException("Missing user email");
            }
            if (this.authUtil.userPassword == null) {
                throw new IllegalStateException("Missing user password");
            }
            if (this.authUtil.host == null) {
                throw new IllegalStateException("Missing host");
            }
            if (!this.authUtil.isAdmin && this.authUtil.documentKind == null) {
    ...
    private void queryUser() {
        ...
        Operation postQuery = Operation.createPost(queryTaskUri)
                ...
                .setCompletion((op, ex) -> {
                    if (ex != null) {
                        this.host.log(Level.WARNING, "Could not query user %s: %s",
                                this.userEmail, ex);
                        return;
                    }
    ...
    private void makeUser() {
        ...
        Operation postUser = Operation.createPost(userFactoryUri)
                ...
                .setCompletion((op, ex) -> {
                    if (ex != null) {
                        this.host.log(Level.WARNING, "Could not make user %s: %s", this.userEmail,
                                ex);
                        return;
                    }
    ...
    private void makeCredentials() {
        ...
        Operation postCreds = Operation.createPost(credentialFactoryUri)
                ...
                .setCompletion((op, ex) -> {
                    if (ex != null) {
                        this.host.log(Level.WARNING, "Could not make credentials for user %s: %s",
                                this.userEmail, ex);
                        return;
                    }
    ...
    private void makeUserGroup() {
        ...
        Operation postGroup = Operation.createPost(userGroupFactoryUri)
                ...
                .setCompletion((op, ex) -> {
                    if (ex != null) {
                        this.host.log(Level.WARNING, "Could not make user group for user %s: %s",
                                this.userEmail, ex);
                        return;
                    }
    ...
    private void makeRole() {
        ...
        Operation postGroup = Operation.createPost(resourceGroupFactoryUri)
                ...
                .setCompletion((op, ex) -> {
                    if (ex != null) {
                        this.host.log(Level.WARNING, "Could not make role for user %s: %s",
                                this.userEmail, ex);
                        return;
                    }
