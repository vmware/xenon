...
/**
 * A service which allow to attach remote XENON services to a node via WebSocket connection. For every attached service
 * actual service implementation lives on a remote client attached to a not via WebSocket (for example, in a web
 * browser).
 * <p/>
 * All such services are exposed by XENON node as /core/ws-service/{some-uuid}. Service links are always generated by
