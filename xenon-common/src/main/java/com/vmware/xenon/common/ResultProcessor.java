...
public final class ResultProcessor {
    ...
    private ResultProcessor(QueryTask task, ServiceDocumentQueryResult results) {
        this.task = task;
        this.results = results;
    }

    public static ResultProcessor create(QueryTask task) {
        return new ResultProcessor(task, task.results);
    }

    public static ResultProcessor create(ServiceDocumentQueryResult results) {
        return new ResultProcessor(null, results);
    }

    /**
     * Create a ResultProcessor by trying to parse the Operation body as either QueryTask or
     ...
     */
    public static ResultProcessor create(Operation op) {
        ...
        if (!task.documentKind.equals(QueryTask.KIND)) {
            ...
            if (ServiceDocumentQueryResult.KIND.equals(r.documentKind) ||
                ...
                return new ResultProcessor(null, r);
            } else {
                throw new IllegalArgumentException(
                        "Cannot create ResultProcessor from a " + r.documentKind + " document");
            }
        } else {
            return new ResultProcessor(task, task.results);
        }
