...
public class NettyHttpListener implements ServiceRequestListener {
    ...
    public void setSSLContextFiles(URI certFile, URI keyFile, String keyPassphrase) throws Throwable {
        ...
        if (OpenSsl.isAlpnSupported()) {
            ...
            this.host.log(Level.WARNING, "OpenSSL is available but does not support ALPN");
            provider = SslProvider.JDK;
        } else {
            provider = SslProvider.JDK;
        }

        this.host.log(Level.INFO, "Using SSL provider: %s", provider);

        this.sslContext = SslContextBuilder
                .forServer(new File(certFile), new File(keyFile), keyPassphrase)
                .sslProvider(provider)
