...
public class ServiceResourceTracker {
    ...
    private void updateCachedServiceState(Service s, ServiceDocument st, Operation op, boolean checkVersion) {
        ...
        // (3) the service has better state from peer after synchronization and state caching is enabled
        boolean cacheState = !ServiceHost.isServiceIndexed(s);
        cacheState |= (!op.isFromReplication() || op.isSynchronizePeer()) && this.isServiceStateCaching;
        if (!cacheState) {
            return;
        }
