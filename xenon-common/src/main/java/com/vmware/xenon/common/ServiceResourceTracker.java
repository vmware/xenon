...
class ServiceResourceTracker {
    ...
    private void updateStats(long now) {
        ...
        ScheduledThreadPoolExecutor scheduledExecutor = (ScheduledThreadPoolExecutor)
                this.host.getCoreScheduledExecutor();
        if (scheduledExecutor != null) {
            mgmtService.setStat(
                    ServiceHostManagementService.STAT_NAME_CORE_SCHEDULED_EXECUTOR_QUEUE_DEPTH,
                    scheduledExecutor.getQueue().size());
        }

        ScheduledThreadPoolExecutor serviceScheduledExecutor = (ScheduledThreadPoolExecutor)
                this.host.getScheduledExecutor();
        if (serviceScheduledExecutor != null) {
            mgmtService.setStat(
                    ServiceHostManagementService.STAT_NAME_SERVICE_SCHEDULED_EXECUTOR_QUEUE_DEPTH,
                    serviceScheduledExecutor.getQueue().size());
        }
    }
