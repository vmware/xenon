...
class ServiceResourceTracker {
    ...
    public void updateCachedServiceState(Service s,
        ...
        if (ServiceHost.isServiceIndexed(s) && !isTransactional(op)) {
            ...
            if (s.getSelfLink().contains("on-demand-load-services")) {
                this.host.log(Level.INFO, "adding lastAccessTime for %s", s.getSelfLink());
            }
        }
    ...
    public ServiceDocument getCachedServiceState(String servicePath, Operation op) {
        ...
        if (s != null && ServiceHost.isServiceIndexed(s) && !isTransactional(op)) {
        ...
        for (Service service : this.attachedServices.values()) {
            ...
            if (lastAccessTime != null) {
                ...
                if (hostState.lastMaintenanceTimeUtcMicros - lastAccessTime < service
                    ...
                    if (service.getSelfLink().contains("on-demand-load-services")) {
                        this.host.log(Level.INFO, "skipping stop for: %s", service.getSelfLink());
                    }
                    continue;
                } else {
                    this.persistedServiceLastAccessTime.remove(service.getSelfLink());
                    if (service.getSelfLink().contains("on-demand-load-services")) {
                        this.host.log(Level.INFO, "We are not skipping stop for: %s", service.getSelfLink());
                    }
                }
            }  else {
                if (service.getSelfLink().contains("on-demand-load-services")) {
                    this.host.log(Level.INFO, "missing lastAccessTime for: %s", service.getSelfLink());
                }
            }
    ...
    private void pauseServices() {
        ...
        this.host.log(Level.INFO, "Paused %d services, attached: %d, cached: %d, persistedServiceLastAccessTimes: %d",
                ...
                this.cachedServiceStates.size(), this.persistedServiceLastAccessTime.size());
    }
    ...
    public void close() {
        ...
        this.persistedServiceLastAccessTime.clear();
    }
