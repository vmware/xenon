...
class ServiceResourceTracker {
    public static ServiceResourceTracker create(ServiceHost host) {
        ServiceResourceTracker sst = new ServiceResourceTracker();
        sst.host = host;
        return sst;
    }

    private ConcurrentSkipListMap<String, ServiceDocument> cachedServiceStates = new ConcurrentSkipListMap<>();
    ...
    public void pauseService(Service s) {

    }

    public Service cancelServicePause(String path) {
        return this.pendingPauseServices.remove(path);
    }
     ...
     */
    public void performMaintenance(long deadlineMicros) {
        ...
        for (Service service : this.attachedServices.values()) {
            ...
            if (!this.host.isServiceIndexed(service)) {
            ...
            if (s != null) {
                if ((hostState.serviceCacheClearDelayMicros + s.documentUpdateTimeMicros) < Utils
                        .getNowMicrosUtc()) {
        ...
        for (Service s : this.pendingPauseServices.values()) {
            ...
            this.host.sendRequest(ServiceContextIndexService.createPost(this.host, path, s)
                    .setReferer(this.host.getUri()).setCompletion((o, e) -> {
                        ...
                        Service serviceEntry = cancelServicePause(path);
