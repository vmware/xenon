...
public class StatefulService implements Service {
    ...
    private static boolean FORCE_SEQUENTIAL_INDEXING = XenonConfiguration.bool(
            StatefulService.class,
            "forceSequentialIndexing",
            false
    );
    ...
    private void processCompletionStageIndexing(Operation op) {
        try {
            op.nestCompletion((o, failure) -> {
                ...
                if (this.hasOption(ServiceOption.SEQUENTIAL_INDEXING)) {
                    if (!op.isWithinTransaction()) {
                        processPending(op);
                    }
                }
            ...
            if (!this.hasOption(ServiceOption.SEQUENTIAL_INDEXING) && !op.isWithinTransaction()) {
                processPending(op);
            }
        }
    }
    ...
    public void toggleOption(ServiceOption option, boolean enable) {
        ...
        if (option == ServiceOption.PERSISTENCE && enable) {
            ...
            if (FORCE_SEQUENTIAL_INDEXING) {
                toggleOption(ServiceOption.SEQUENTIAL_INDEXING, true);
            }
        }
