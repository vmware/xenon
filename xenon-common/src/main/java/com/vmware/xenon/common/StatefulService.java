...
public class StatefulService implements Service {
    ...
    private boolean queueSynchronizedRequest(final Operation op) {
        ...
        if (op.getAction() != Action.GET && op.getAction() != Action.OPTIONS) {
            ...
            if (op.getAction() == Action.OPTIONS) {
                return false;
            } else if (isIndexed() && hasOption(ServiceOption.CONCURRENT_GET_HANDLING)) {
    ...
    private void processPending(Operation op) {
        ...
        if (op.getAction() == Action.GET) {
            if (isIndexed() && hasOption(ServiceOption.CONCURRENT_GET_HANDLING)) {
    ...
    public void toggleOption(ServiceOption option, boolean enable) {
        ...
        if (option == ServiceOption.CONCURRENT_GET_HANDLING
                && this.context.options.contains(ServiceOption.CONCURRENT_UPDATE_HANDLING)) {
            logWarning("ServiceOption.CONCURRENT_UPDATE_HANDLING implies ServiceOption.CONCURRENT_GET_HANDLING");
        }
