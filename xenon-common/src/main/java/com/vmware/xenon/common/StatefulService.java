...
public class StatefulService implements Service {
    ...
    private void handleSynchFailure(Operation request, Operation synchRsp, Throwable e) {
        ...
        if (isMarkedDeleted) {
            this.logWarning("On-demand synch failed because more recent version of service" +
            ...
            Operation stopOp = Operation
                    ...
                    .setCompletion((o, innerEx) -> {
                        if (innerEx != null) {
                            this.logSevere("Failed to stop with failure %s", innerEx.toString());
                        }
                        failRequest(request, e, false);
                        return;
                    });
            this.handleRequest(stopOp);
            return;
        }
