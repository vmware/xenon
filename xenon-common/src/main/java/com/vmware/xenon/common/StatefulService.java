...
public class StatefulService implements Service {
    ...
    private boolean validateOwnerSelectedUpdate(Operation request) {
        ...
        logInfo("Processing op with ID %d, epoch %d, version %d, current epoch %d, version %d",
                request.getId(), stateFromOwner.documentEpoch, stateFromOwner.documentVersion,
                this.context.epoch, this.context.version);

        if (stateFromOwner.documentEpoch == this.context.epoch
                && stateFromOwner.documentVersion > this.context.version + 1) {
