...
public class SynchronizationTaskService
    ...
    public void handlePut(Operation put) {
        ...
        case STARTED:
            ...
            // i.e. QUERY.
            //logInfo("DEBUG: Restarting SynchronizationTask");
        ...
        if (startStateMachine) {
            ...
            boolean debug = task.factorySelfLink.equals(ExampleService.FACTORY_LINK);
            if (debug) {
                logInfo("DEBUG: Starting state machine");
            }
    ...
    public void handlePatch(Operation patch) {
        ...
        long timeFromStartMillis = TimeUnit.MICROSECONDS.toMillis(now - task.startTimeMicros);
        if (debug) {
            logInfo("DEBUG: Transitioning task from %s-%s to %s-%s, Services synchronized: %d",
                    currentStage, currentSubStage, task.taskInfo.stage, task.subStage, task.synchCompletionCount);
            logInfo("DEBUG: timeFromStart (millis): %d", timeFromStartMillis);
        }
    ...
    private QueryTask buildChildQueryTask(State task) {
        ...
        if (this.parent != null && this.parent.hasChildOption(ServiceOption.PERSISTENCE) &&
                this.isCheckpointEnabled && task.checkpoint != null) {
            boolean debug = task.factorySelfLink.equals(ExampleService.FACTORY_LINK);
            if (debug) {
                log(Level.INFO, "DEBUG: query %s from checkpoint %d\n", task.factorySelfLink, task.checkpoint);
            }
        ...
        Operation getNodeSelectorStateOp = Operation
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (this.parent != null && this.parent.hasChildOption(ServiceOption.PERSISTENCE)
                        ...
                        boolean debug = task.factorySelfLink.equals(ExampleService.FACTORY_LINK);
                        if (debug) {
                            logInfo("DEBUG: create next checkpoint for factory %s using timestamp %d",
                                    this.parent.getSelfLink(), task.startTimeMicros);
                        }
