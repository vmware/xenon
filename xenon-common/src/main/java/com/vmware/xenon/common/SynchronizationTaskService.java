...
public class SynchronizationTaskService
    ...
    protected State validateStartPost(Operation post) {
        ...
        if (task.taskInfo.stage != null && task.taskInfo.stage != TaskState.TaskStage.CREATED &&
                task.taskInfo.stage != TaskState.TaskStage.CANCELLED) {
            post.fail(
                    new IllegalArgumentException(
                            "synch-tasks can only be created in CREATED or CANCELLED states"));
        }
    ...
    public void handlePut(Operation put) {
        ...
        // If this is a cancellation request. update the task state to cancelled
        // and complete the operation. If the task was running, it will get preempted
        // in the next self-patch to stop processing.
        if (body.taskInfo.stage == TaskState.TaskStage.CANCELLED) {
            task.membershipUpdateTimeMicros = body.membershipUpdateTimeMicros;
            task.taskInfo.stage = TaskState.TaskStage.CANCELLED;
            task.subStage = null;
            put.complete();
    ...
    public void handlePatch(Operation patch) {
        ...
        // Synch-tasks support cancellation because of scenarios where the
        // current node may not be the factory owner anymore. If the task
        // was in cancelled state, then ignore the PATCH to stop the task
        // from running.
        if (task.taskInfo.stage == TaskState.TaskStage.CANCELLED) {
            patch.complete();
            return;
        }
    ...
    protected boolean validateTransition(Operation patch, State currentTask, State patchBody) {
        ...
        if (patchBody.taskInfo.stage == TaskState.TaskStage.CANCELLED) {
            patch.complete();
        }
        if (currentTask.taskInfo != null && currentTask.taskInfo.stage != null) {
            if (currentTask.taskInfo.stage != TaskState.TaskStage.CANCELLED &&
                    currentTask.taskInfo.stage.ordinal() > patchBody.taskInfo.stage.ordinal()) {
    ...
    public void handleSubStage(State task) {
        logInfo("Syncing %s, Stage %s, SubStage %s", task.factorySelfLink, task.taskInfo.stage, task.subStage);
...
//                    }
//
//                    // Local node thinks it's the owner. Let's confirm that majority of the nodes
//                    // also agree with that.
//                    NodeSelectorService.SelectAndForwardRequest request =
//                            new NodeSelectorService.SelectAndForwardRequest();
//                    request.key = task.factorySelfLink;
//
//                    Operation broadcastSelectOp = Operation
//                            .createPost(UriUtils.buildUri(this.getHost(), task.nodeSelectorLink))
//                            .setReferer(this.getHost().getUri())
//                            .setExpiration(task.documentExpirationTimeMicros)
//                            .setBody(request)
//                            .setCompletion((op, t) -> {
//                                if (t != null) {
//                                    sendSelfFailurePatch(task, t.getMessage());
//                                    return;
//                                }
//
//                                // counting local node.
...
//                                for (String json : response.jsonResponses.values()) {
//                                    NodeSelectorService.SelectOwnerResponse ownerResponse =
//                                            Utils.fromJson(json, NodeSelectorService.SelectOwnerResponse.class);
//                                    if (ownerResponse.ownerNodeId.equals(this.getHost().getId())) {
//                                        consensusCount++;
//                                    }
//                                }
//
//                                if (consensusCount < (rsp.availableNodeCount / 2) + 1) {
//                                    sendSelfCancellationPatch(task,
//                                            "Majority nodes do not consider the current node owner for this factory");
//                                    return;
//                                }
//
//                                handleSynchronizeChildren(task, false);
//                            });
//
//                    getHost().broadcastRequest(task.nodeSelectorLink, task.factorySelfLink, true, broadcastSelectOp);
//                });
//
