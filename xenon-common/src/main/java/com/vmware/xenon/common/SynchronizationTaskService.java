...
public class SynchronizationTaskService
    ...
    private void handleCheckpointStage(State task) {
        ...
        Operation get = Operation.createGet(UriUtils.buildUri(this.getHost(), checkPointServiceLink))
                ...
                .setCompletion((o, e) -> {
                    if (e != null) {
                        log(Level.INFO, "broadcast get checkpoints fail %s, starting synchronization from timestamp 0", e.toString());
                        task.checkpoint = 0L;
                    ...
                    if (!rsp.failures.isEmpty()) {
                        for (Map.Entry<URI, ServiceErrorResponse> failure : rsp.failures.entrySet()) {
                            log(Level.INFO, "get checkpoint fail with status %d from %s", failure.getValue().errorCode, failure.getKey());
                        }
    ...
    private void getCheckpoints(Operation maintOp) {
        ...
        Operation get = Operation.createGet(UriUtils.buildUri(this.getHost(), checkPointServiceLink))
                ...
                .setCompletion((o, e) -> {
                    if (e != null) {
                        log(Level.INFO, "broadcast get checkpoints fail %s", e.toString());
                        maintOp.fail(e);
                        return;
                    }
                    NodeGroupBroadcastResponse rsp = o.getBody(NodeGroupBroadcastResponse.class);
                    ...
                    if (!rsp.failures.isEmpty()) {
                        for (Map.Entry<URI, ServiceErrorResponse> failure : rsp.failures.entrySet()) {
                            if (failure.getValue().statusCode != Operation.STATUS_CODE_NOT_FOUND) {
                                log(Level.INFO, "get checkpoint fail with status %d from %s", failure.getValue().errorCode, failure.getKey());
                                maintOp.fail(new IllegalStateException(String.format("get checkpoint fail with status %d from %s", failure.getKey(), failure.getValue().statusCode)));
                                return;
                            }
                        }
                    ...
                    log(Level.INFO, "prev min checkpoint(%d)", prevMinCheckpoint);
    ...
    private void scanCheckpoint(long prevMinCheckpoint, Map<String, URI> selectedNodes, Set<URI> receivers, Operation maintOp) {
        ...
        Operation.createPost(forwardingService)
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (!rsp.failures.isEmpty()) {
                        for (Map.Entry<URI, ServiceErrorResponse> failure : rsp.failures.entrySet()) {
                            log(Level.INFO, "Query checkpoint fail with status %d from %s", failure.getValue().statusCode, failure.getKey());
                        }
    ...
    private QueryTask buildScanQueryTask(long prevMinCheckpoint) {
        QueryTask.NumericRange<Long> r =
                QueryTask.NumericRange.createLongRange(prevMinCheckpoint, Utils.getNowMicrosUtc() - this.parent.getCheckpointComparisonEpsilonMicros(),
                        false, true);
        ...
        return QueryTask.Builder.createDirectTask()
                .setQuery(q)
                .setResultLimit(this.parent.getCheckpointQueryLimit())
