...
public class SynchronizationTaskService
    ...
    public static class State extends TaskService.TaskServiceState {
        ...
        /**
         * start time of a synchronization task
         */
    ...
    /**
     * period to schedule the next synchronization task
     */
    private final long synchronizationSchedulePeriodInMicros = XenonConfiguration.number(
            SynchronizationTaskService.class,
    ...
    public void handlePut(Operation put) {
        ...
        task.startTimeMicros = body.startTimeMicros;
    ...
    private void handleCheckNodeGroupAvailabilityStage(State task) {
        ...
        Operation getNodeSelectorStateOp = Operation
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (this.parent != null && this.parent.hasChildOption(ServiceOption.PERSISTENCE) && this.isCheckpointEnabled) {
                        if (task.membershipUpdateTimeMicros.equals(nsState.membershipUpdateTimeMicros)) {
                            ...
                            log(Level.INFO, "create next checkpoint for factory %s using timestamp %d", this.parent.getSelfLink(), task.startTimeMicros);

                            Operation post = Operation.createPost(UriUtils.buildUri(this.getHost(), CheckpointService.FACTORY_LINK))
                                    .setBody(s)
                                    .setReferer(this.getUri())
                                    .setCompletion((op, ex) -> {
                                        // schedule itself in period
                                        this.getHost().scheduleCore(() -> {
                                            // ownership check
                                            long ownershipCheckExpiration = Utils.fromNowMicrosUtc(getHost().getOperationTimeoutMicros());
                                            Utils.checkAndUpdateDocumentOwnership(getHost(), this, ownershipCheckExpiration, (ownershipOp, ownershipEx) -> {
                                                if (ownershipEx != null) {
                                                    return;
                                                }
                                                ...
                                                log(Level.INFO, "synchronization task for %s scheduled at %s",
                                                        this.parent.getSelfLink(), this.getHost().getId());
                                                SynchronizationTaskService.State scheduleTask = createSynchronizationTaskState(
                                                        task.membershipUpdateTimeMicros);
                                                Operation.createPost(this, ServiceUriPaths.SYNCHRONIZATION_TASKS)
                                                        .setBody(scheduleTask)
                                                        .sendWith(this);
                                            });
                                        }, this.synchronizationSchedulePeriodInMicros, TimeUnit.MICROSECONDS);
    ...
    private SynchronizationTaskService.State createSynchronizationTaskState(
        ...
        task.documentSelfLink = UriUtils.convertPathCharsFromLink(this.parent.getSelfLink());
        task.factorySelfLink = this.parent.getSelfLink();
        task.factoryStateKind = Utils.buildKind(this.parent.getStateType());
        task.membershipUpdateTimeMicros = membershipUpdateTimeMicros;
        task.startTimeMicros = Utils.getNowMicrosUtc();
        task.nodeSelectorLink = this.parent.getPeerNodeSelectorPath();
        task.queryResultLimit = this.parent.getSelfQueryResultLimit();
