...
public class SynchronizationTaskService extends TaskService<SynchronizationTaskService.State> {
    ...
    public static final String STAT_NAME_WAIT_FOR_NG_AVAILABLE_RETRY_COUNT = "ngAvailableRetryCount";
    ...
    public enum SubStage {
        QUERY, SYNCHRONIZE, RESTART, WAIT_FOR_NG_AVAILABLE
    }
    ...
    public void handleSubStage(State task) {
        ...
        case SYNCHRONIZE:
            ...
            break;
        case WAIT_FOR_NG_AVAILABLE:
            handleWaitForNodeGroupAvailableStage(task);
            break;
        default:
    ...
    private void handleSynchronizeStage(State task, boolean verifyOwnership) {
        if (task.queryPageReference == null) {
            sendSelfPatch(task, TaskState.TaskStage.STARTED,
                    subStageSetter(SubStage.WAIT_FOR_NG_AVAILABLE));
            return;
        }
    ...
    private void handleWaitForNodeGroupAvailableStage(State task) {
        Operation get = Operation.createGet(getHost(), ServiceUriPaths.DEFAULT_NODE_GROUP)
                .setCompletion((o, e) -> {
                    ...
                    scheduleRetry(() -> {
                        sendSelfPatch(task, TaskState.TaskStage.STARTED,
                                subStageSetter(SubStage.WAIT_FOR_NG_AVAILABLE));
                    }, STAT_NAME_WAIT_FOR_NG_AVAILABLE_RETRY_COUNT);
                });
        sendRequest(get);
    }
