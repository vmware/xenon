...
public class SynchronizationTaskService
    ...
    public static class State extends TaskService.TaskServiceState {
        ...
        /**
         * upper bound for checkpoint document
         */
        public Long checkpointUpdateTimeMicros;
    ...
    public void handlePut(Operation put) {
        ...
        // previous task is not finished successfully
        boolean retry = !(task.taskInfo.stage == TaskState.TaskStage.CREATED ||
                task.taskInfo.stage == TaskState.TaskStage.FINISHED);
        if (!retry) {
            task.checkpointUpdateTimeMicros = body.membershipUpdateTimeMicros;
        }
    ...
    private void handleCheckpointStage(State task) {
        QueryTask queryTask = buildCheckpointQueryTask(task);
        ...
        Operation.createPost(forwardingService)
                ...
                .setCompletion((o, e) -> {
                    if (e != null) {
                        log(Level.INFO, "broadcast query checkpoints failed %s, starting synchronization from timestamp 0", e.toString());
                        task.checkpoint = 0L;
                    ...
                    for (URI uri : rsp.jsonResponses.keySet()) {
                        ...
                        if (qt.results.documentCount > 0) {
                            CheckpointService.CheckpointState cps =
                                    Utils.fromJson(qt.results.documents.values().iterator().next(), CheckpointService.CheckpointState.class);
                            log(Level.INFO, "checkpoint %d from %s for %s", cps.timestamp, uri, task.factorySelfLink);
                            minCheckpoint = Long.min(minCheckpoint, cps.timestamp);
                        } else {
                            log(Level.INFO, "no checkpoint before %d from %s for %s", task.membershipUpdateTimeMicros, uri, task.factorySelfLink);
                            minCheckpoint = 0L;
                            break;
                        }
                    }
                    task.checkpoint = minCheckpoint;
                    sendSelfPatch(task, TaskState.TaskStage.STARTED,
                            subStageSetter(SubStage.QUERY));
                    return;
                })
    ...
    private QueryTask buildCheckpointQueryTask(State task) {
        QueryTask.NumericRange<Long> r =
                QueryTask.NumericRange.createLongRange(0L, task.checkpointUpdateTimeMicros,
                        true, false);
        QueryTask.Query q = QueryTask.Query.Builder.create()
                .addKindFieldClause(CheckpointService.CheckpointState.class)
                .addFieldClause(CheckpointService.CheckpointState.FIELD_NAME_FACTORY_LINK, task.factorySelfLink)
                .addRangeClause(ServiceDocument.FIELD_NAME_UPDATE_TIME_MICROS, r)
                .build();
        return QueryTask.Builder.createDirectTask()
                .setQuery(q)
                .setResultLimit(1)
                .orderDescending(ServiceDocument.FIELD_NAME_UPDATE_TIME_MICROS, ServiceDocumentDescription.TypeName.LONG)
