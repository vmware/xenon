...
public class SynchronizationTaskService
                ...
                .setCompletion((o, e) -> {
                    ...
                    for (Entry<URI, String> entry: rsp.jsonResponses.entrySet()) {
                        CheckpointService.CheckpointState checkpointState =
                                Utils.fromJson(entry.getValue(), CheckpointService.CheckpointState.class);
                        logInfo("synch %s from checkpoint: host %s returned %d",
                                task.factorySelfLink, entry.getKey(), checkpointState.timestamp);
                    }
    ...
    private boolean checkpointEnabled(State task) {
        return this.isCheckpointEnabled &&
                !task.childDocumentIndexLink.equals(ServiceUriPaths.CORE_IN_MEMORY_DOCUMENT_INDEX);
    }
}
