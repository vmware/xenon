...
public class SynchronizationTaskService
    ...
    private void scheduleTaskRetryOnFailure(
            Runnable task, Runnable failure, String retryCountStatName, String failureCounterStatName) {

        adjustStat(retryCountStatName, 1);
        ServiceStats.ServiceStat stat = getStat(retryCountStatName);
        if (stat != null) {
            if (stat.latestValue > MAX_SYNCH_RETRY_COUNT) {
                adjustStat(failureCounterStatName, 1);
                logSevere("Retry counter (%s), exhausted maximum %d retires",
                        retryCountStatName, (long) stat.latestValue);
                failure.run();
                return;
            }
        }
        ...
        logWarning("Scheduling retry of task with counter %s:%d in %d seconds",
                retryCountStatName, stat != null ? (int)stat.latestValue : 0, TimeUnit.MICROSECONDS.toSeconds(delay));
