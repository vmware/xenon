...
public class TracerFactory {
    ...
    public synchronized Tracer create(ServiceHost host) {
        ...
        Class<FactoryInterface> factoryClass = null;
        try {
            if (implementation.equals("jaeger")) {
                factoryClass = (Class<FactoryInterface>) Class.forName("com.vmware.xenon.common.opentracing.Jaeger");
            } else if (implementation.equals("zipkin")) {
                factoryClass = (Class<FactoryInterface>) Class.forName("com.vmware.xenon.common.opentracing.Zipkin");
            } else {
                throw new RuntimeException(String.format("Bad tracer type %s", implementation));
            }
            if (factoryClass == null) {
                throw new RuntimeException(String.format(
                        "Failed to cast implementation '%s' to Class<FactoryInterface>", implementation));
            }
        } catch (ClassNotFoundException e) {
            logger.log(Level.SEVERE, "Failed to load implementation class", e);
            throw new RuntimeException(String.format("Could not load implementation for %s", implementation));
        }
        ...
        try {
            ...
            throw new RuntimeException(String.format("Could not instantiate factory for %s", implementation));
        }
