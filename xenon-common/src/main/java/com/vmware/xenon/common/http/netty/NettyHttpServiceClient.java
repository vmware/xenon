...
public class NettyHttpServiceClient implements ServiceClient {
    ...
    private void sendRemote(Operation op) {
        ...
        op.nestCompletion((o, e) -> {
            LOGGER.info(String.format("sendRemote Operation %d complete with status %d error %s", o.getId(), o.getStatusCode(), e != null ? e.getMessage() : "NULL"));
    ...
    private void connectChannel(NettyChannelPool pool, Operation op,
        ...
        op.nestCompletion((o, e) -> {
            LOGGER.info(String.format("connectChannel Operation %d complete with status %d, error %s", o.getId(), o.getStatusCode(), e != null ? e.getMessage() : "NULL"));
            if (o.getStatusCode() == Operation.STATUS_CODE_TIMEOUT) {
    ...
    private void sendHttpRequest(Operation op) {
        ...
        try {
            ...
            op.nestCompletion((o, e) -> {
                LOGGER.info(String.format("sendHttpRequest Operation %d complete with status %d, error %s", o.getId(), o.getStatusCode(), e != null ? e.getMessage() : "NULL"));
                if (o.getStatusCode() == Operation.STATUS_CODE_TIMEOUT) {
                    o.fail(new TimeoutException());
                    return;
                }
