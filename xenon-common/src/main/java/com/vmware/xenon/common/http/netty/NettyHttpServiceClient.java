...
public class NettyHttpServiceClient implements ServiceClient {
    ...
    private SortedMap<Long, Operation> pendingRequests = new TreeMap<>();
    ...
    private void startTracking(Operation op) {
        // FIX BEFORE CHECKIN: this map has collisions sinc multiple ops can have same expiration if the
        // client clones or just sends the same op in a tight loop... use a multimap
        synchronized (this.pendingRequests) {
