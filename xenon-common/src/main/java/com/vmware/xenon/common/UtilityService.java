...
public class UtilityService implements Service {
    ...
    private static class KeyDict {
        ...
        KeyDict() {
            ...
            // durations of specific actions are recorded seperately
            for (Action action : Action.values()) {
                register(action + Service.STAT_NAME_OPERATION_DURATION);
            }
    ...
    private static final KeyDict STATS_KEY_DICT = new KeyDict();
    ...
    private ServiceStat findStat(String name, boolean create, ServiceStat initialStat) {
        synchronized (this.stats) {
            ...
            if (st == null && create) {
                ...
                name = STATS_KEY_DICT.getKey(name);
                st.name = name;
