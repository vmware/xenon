    ...
    public static URI extendUri(URI uri, String path) {
        ...
        if (path != null && path.contains(UriUtils.URI_QUERY_CHAR)) {
            ...
            if (pathAndQuery.length == 2) {
                ...
                throw new IllegalArgumentException(String.format("multiple query characters in path: %s" + path));
            }
        }
            ...
            if (p.contains(UriUtils.URI_QUERY_CHAR)) {
                ...
                if (query == null) {
                    if (pathAndQuery.length == 2) {
                        ...
                        throw new IllegalArgumentException(String.format("multiple query characters in path: %s" + p));
                    }
                } else {
                    if (pathAndQuery.length == 2) {
                        query += pathAndQuery[1];
