...
/**
 * This service logically partitions a namespace into units, called "shards",
 * where each unit is a collection of nodes. The logical namespace contains
 * strings ('keyvalue's), and each string is mapped to shard.
 *
 * It is used by a NodeSelector with a non-null replicationFactory to allocate
 * shards (choosing the nodes within the nodegroup that the shard will consist
 * of), and track node to shard mapping, in order to choose an owner and
 * replication targets from the shard's nodes.
 */
public class ShardsManagementService extends StatefulService {
    ...
    public static final String CHILD_SELFLINK_SUFFIX = UriUtils.buildUriPath(
            FACTORY_LINK_SUFFIX, "manager");
    ...
    public static String getShardIdFromKeyValue(String shardKeyValue, boolean allowShardSharing,
        ...
        int hash = FNVHash.compute32(shardKeyValue);
    ...
    public static class GetMaxShardsRequest {
        public static final String KIND = Utils.buildKind(GetMaxShardsRequest.class);
        public String kind;
    }

    public static class MaxShardsResponse {
        public int maxShards;
    }

    public static class GetAllowShardSharingRequest {
        public static final String KIND = Utils.buildKind(GetAllowShardSharingRequest.class);
        public String kind;
    }

    public static class AllowShardsSharingResponse {
        public boolean allowShardsSharing;
    }
    ...
    private void init(Operation op) {
        State state = op.hasBody() ? op.getBody(State.class) : null;
        if (!validateState(op, state)) {
            return;
        }
    ...
    private boolean validateState(Operation op, State state) {
        String errMsg = null;
        if (state == null) {
    ...
    public void handleGet(Operation get) {
        ...
        GetMaxShardsRequest getMaxShardsRequestBody = get.getBody(GetMaxShardsRequest.class);
        if (GetMaxShardsRequest.KIND.equals(getMaxShardsRequestBody.kind)) {
            MaxShardsResponse responseBody = new MaxShardsResponse();
            responseBody.maxShards = currentState.maxShards;
            get.setBody(responseBody).complete();
            return;
        }

        GetAllowShardSharingRequest getAllowShardSharingRequestBody = get.getBody(GetAllowShardSharingRequest.class);
        if (GetAllowShardSharingRequest.KIND.equals(getAllowShardSharingRequestBody.kind)) {
            AllowShardsSharingResponse responseBody = new AllowShardsSharingResponse();
            responseBody.allowShardsSharing = currentState.allowShardsSharing;
            get.setBody(responseBody).complete();
            return;
        }

        String errorMsg = "unsupported request kind";
        get.fail(Operation.STATUS_CODE_BAD_REQUEST, new IllegalArgumentException(errorMsg), errorMsg);
    }
    ...
    private void handleAddNodes(Operation patch, AddNodesRequest body, State currentState) {
    ...
    private Set<String> allocateNodesForShard(String shardKeyValue, String shardId, State currentState) {
    ...
    private void updateCachedNodeGroupState(NodeGroupState ngs) {
        ...
        addNodesRequest.nodes = ngs.nodes.keySet();
