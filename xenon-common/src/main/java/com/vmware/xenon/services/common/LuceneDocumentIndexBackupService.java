...
public class LuceneDocumentIndexBackupService extends StatelessService {
    ...
    // default selflink is set to backup filebased LuceneDocumentIndexService
    // for in-memory index, register backup service with "ServiceUriPaths.CORE_IN_MEMORY_DOCUMENT_INDEX_BACKUP"
            ...
            Operation uploadOp = Operation.createPost(backupRequest.destination)
                    ...
                    .setCompletion((oop, oox) -> {
                        ...
                        try {
                            ...
                            originalOp.fail(new IllegalStateException("failed to delete backup file "
    ...
    private void restoreFromLocal(Operation op, Path restoreFrom, Long timeSnapshotBoundaryMicros) {
        ...
        boolean isInMemoryIndex = isInMemoryIndex();
        boolean restoreFromZipFile = !Files.isDirectory(restoreFrom);
