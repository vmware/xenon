...
public class GraphQueryTaskService extends TaskService<GraphQueryTask> {
    ...
    protected GraphQueryTask validateStartPost(Operation taskOperation) {
        ...
        if (task.stageSpecs == null || task.stageSpecs.isEmpty()) {
     ...
     * we will issue the first query in the  {@link GraphQueryTask#stageSpecs}.
     *
    ...
    private void startOrContinueGraphQuery(GraphQueryTask currentState,
        ...
        if (lastResults != null) {
            ...
            if (currentState.currentDepth > currentState.depthLimit) {
                // traversal complete. Include the results from the last stage query, directly
                // as part of the state, so the client does not have to GET them independently
                currentState.results = lastResults;
        ...
        QueryTask task = QueryTask.Builder.createDirectTask()
                ...
                .build();
        int traversalSpecIndex = Math.min(currentState.stageSpecs.size() - 1,
                currentState.currentDepth);
        ...
        task.querySpec = currentState.stageSpecs.get(traversalSpecIndex);
        task.documentExpirationTimeMicros = currentState.documentExpirationTimeMicros;
        task.indexLink = currentState.indexLink;
        task.nodeSelectorLink = currentState.nodeSelectorLink;
    ...
    private void handleQueryStageCompletion(GraphQueryTask currentState, Operation o, Throwable e) {
        ...
        if (response.querySpec.resultLimit != null
            ...
            currentState.resultLinks.add(response.results.nextPageLink);
        } else {
            currentState.resultLinks.add(response.documentSelfLink);
        }

        if (response.results.nextPageLink == null
                && response.results.documentCount == 0) {
            currentState.currentDepth++;
            if (currentState.currentDepth < currentState.depthLimit) {
                // query terminated early
                currentState.results = response.results;
                sendSelfPatch(currentState, TaskStage.FINISHED, null);
                return;
            }
        }
        ...
        // in this method, on entry.
        sendSelfPatch(currentState, TaskStage.STARTED, null);
    }

    private void fetchLastStageResults(GraphQueryTask currentState) {
        // retrieve query link. If query is paginated, it will be the link
        // for the next page we need to fetch
        URI queryTaskUri = UriUtils.buildUri(getHost(),
                currentState.resultLinks.get(currentState.currentDepth));
        ...
        CompletionHandler c = (o, e) -> {
            ...
            startOrContinueGraphQuery(currentState, rsp.results);
        };
        Operation.createGet(queryTaskUri)
                .setCompletion(c).sendWith(this);
    }
