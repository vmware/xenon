...
public class MigrationTaskService extends StatefulService {
    ...
    public MigrationTaskService() {
        ...
        super.setMaintenanceIntervalMicros(1000);
    }
    ...
    public void handleStart(Operation startPost) {
        ...
        if (initState.continuousMigration) {
            setMaintenanceIntervalMicros(initState.maintenanceInterval);
        }
    ...
    private State initialize(State initState) {
        ...
        if (initState.maintenanceInterval == null) {
            initState.maintenanceInterval = DEFAULT_MAINTENANCE_INTERVAL_MILLIS;
        }
    ...
    public void handlePatch(Operation patchOperation) {
        ...
        if ((patchState.maintenanceInterval != null || patchState.continuousMigration != null)
                && currentState.continuousMigration) {
            setMaintenanceIntervalMicros(currentState.maintenanceInterval);
        }
            ...
            .setCompletion((o, t) -> {
                ...
                if (state.continuousMigration
                    ...
                    if (state.taskInfo.stage == TaskStage.FINISHED) {
                        ...
                        // remove any numeric clauses on the update time field since we use a numeric clause over that
                        // field to determine, which documents to migrate
                        for (Query q : patch.querySpec.query.booleanClauses) {
                            if (q.term.propertyName.equals(ServiceDocument.FIELD_NAME_UPDATE_TIME_MICROS)
                                    || q.term.range != null) {
                                patch.querySpec.query.booleanClauses.remove(q);
                            }
                        }
    ...
    private void scheduleWaitUntilNodeGroupsAreStable(State currentState, int allowedConvergenceChecks, Runnable onSuccess) {
        ...
        }, currentState.maintenanceInterval, TimeUnit.MILLISECONDS);
    }
