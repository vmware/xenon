...
/**
 * Copy of {@link org.apache.lucene.codecs.lucene60.Lucene60FieldInfosFormat}. The original is final hooks
 * cannot be attached. The only hook need is caching of FieldInfo instances. The changed code is delimited
 * with ///////////////////.
 *
 */
public final class Lucene60FieldInfosFormatWithCache extends FieldInfosFormat {
    ...
    private static final class FieldInfoKey {
        ...
        public int hashCode() {
            int h = 17;
            h = h * 31 + this.fieldInfo.number;
            h = h * 31 + this.fieldInfo.name.hashCode();
            h = h * 31 + this.fieldInfo.getPointDimensionCount();
            h = h * 31 + this.fieldInfo.getPointNumBytes();
            h = h * 31 + (int) this.fieldInfo.getDocValuesGen();
            h = h * 31 + this.fieldInfo.getIndexOptions().hashCode();
            return h;
        }
        ...
        public boolean equals(Object obj) {
            ...
            return this.fieldInfo.number == that.fieldInfo.number &&
                    this.fieldInfo.name.equals(that.fieldInfo.name) &&
                    this.fieldInfo.getPointDimensionCount() == this.fieldInfo.getPointDimensionCount() &&
                    this.fieldInfo.getPointNumBytes() == this.fieldInfo.getPointNumBytes() &&
                    this.fieldInfo.getDocValuesGen() == that.fieldInfo.getDocValuesGen() &&
                    this.fieldInfo.getIndexOptions() == that.fieldInfo.getIndexOptions();
        }
    }

    private final ConcurrentMap<FieldInfoKey, FieldInfo> cachedFields = new ConcurrentHashMap<>();

    private FieldInfo getUnique(FieldInfo fi) {
        FieldInfoKey key = new FieldInfoKey(fi);
        FieldInfo old = this.cachedFields.putIfAbsent(key, fi);
