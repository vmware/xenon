...
public final class Lucene60FieldInfosFormatWithCache extends FieldInfosFormat {
    ...
    private final ConcurrentMap<FieldInfoKey, FieldInfo> cachedInfo = new ConcurrentHashMap<>();

    private final ConcurrentMap<FieldInfosKey, FieldInfos> cachedInfos = new ConcurrentHashMap<>();

    private FieldInfo dedupFieldInfo(FieldInfo fi) {
        FieldInfoKey key = new FieldInfoKey(fi);
        FieldInfo old = this.cachedInfo.putIfAbsent(key, fi);
    ...
    private FieldInfos dedupFieldInfos(FieldInfo[] infos) {
        ...
        return this.cachedInfos.computeIfAbsent(key, k -> new FieldInfos(k.infos));
    }
