    ...
    */
    private static void mergeForReadAfterWriteConsistency(
        ...
        if (nodeGroupResponse.membershipQuorum < ((nodeGroupResponse.nodeCount / 2) + 1 )) {
            onCompletion.accept(null, new IllegalStateException("Membership quorum value should be set to be"
        ...
        for (String getLink : getLinks) {
            getOps.add(Operation
                    .createGet(UriUtils.buildUri(host, getLink))
