...
public class ODataQueryService extends StatelessService {
    ...
    public void handleGet(Operation op) {
        ...
        try {
            ...
            sendRequest(Operation.createPost(this, ServiceUriPaths.CORE_QUERY_TASKS).setBody(task).setCompletion((o, e) -> {
                if (e != null) {
                    op.fail(e);
                    return;
                }
    ...
    private void handleOdataPaginationCompletion(final Operation op, final String skipTo) {
        ...
        if (node == null) {
            op.fail(new IllegalArgumentException(String.format(" When using %s , %s must be supplied",
                                                               UriUtils.URI_PARAM_ODATA_SKIP_TO,
                                                               UriUtils.URI_PARAM_ODATA_NODE)));
            return;
        }
        URI u = UriUtils.buildUri(getHost(), UriUtils.buildUriPath(ServiceUriPaths.CORE, "query-page", skipTo + ""));
        URI forwarderUri = UriUtils.buildForwardToPeerUri(u, node, ServiceUriPaths.DEFAULT_NODE_SELECTOR, EnumSet.noneOf(ServiceOption.class));
