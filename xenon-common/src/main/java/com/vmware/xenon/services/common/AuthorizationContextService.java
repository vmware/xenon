...
public class AuthorizationContextService extends StatelessService {
    ...
    private final Set<String> cacheClearRequests = new HashSet<>();
    ...
    private boolean handleCacheClearRequest(Operation op, String subject) {
        ...
        synchronized (this.cacheClearRequests) {
    ...
    private void populateAuthorizationContext(AuthorizationContext ctx, Claims claims,
        ...
        try {
            ...
            boolean retry = false;
            synchronized (this.cacheClearRequests) {
                if (this.cacheClearRequests.contains(claims.getSubject())) {
                    this.cacheClearRequests.remove(claims.getSubject());
                    retry = true;
                }
            }
            if (retry) {
