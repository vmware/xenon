...
public class NodeGroupService extends StatefulService {
    ...
    private NodeState buildLocalNodeState(NodeState body) {
        ...
        Integer membershipQuorum = Integer.getInteger(NodeState.PROPERTY_NAME_MEMBERSHIP_QUORUM);
        if (membershipQuorum != null) {
            body.membershipQuorum = membershipQuorum;
        } else {
        ...
        if (getHost().getState().location != null) {
            body.customProperties.put(NodeState.PROPERTY_NAME_LOCATION,
                    getHost().getState().location);
            logInfo("Setting node state location to %s", getHost().getState().location);
        }
