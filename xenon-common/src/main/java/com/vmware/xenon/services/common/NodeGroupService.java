...
public class NodeGroupService extends StatefulService {
    ...
    private NodeState buildLocalNodeState(NodeState body) {
        ...
        if (getHost().getState().datacenter != null) {
            body.customProperties.put(NodeState.PROPERTY_NAME_DATACENTER,
                    getHost().getState().datacenter);
            logInfo("Setting node state datacenter to %s", getHost().getState().datacenter);
        }
