...
public class ConsistentHashingNodeSelectorService extends StatelessService implements
    ...
    private boolean queueRequestIfNodeGroupIsUnavailable(NodeGroupState localState,
        ...
        if (!NodeSelectorState.AVAILABLE.contains(NodeSelectorState.checkStatus(this.cachedState))) {
    ...
    private void performPendingRequestMaintenance() {
        ...
        while (!this.pendingRequests.isEmpty()) {
            if (! NodeSelectorState.AVAILABLE.contains(NodeSelectorState.calculateStatus(getHost(), this.cachedGroupState))) {
    ...
    private void updateCachedNodeGroupState(NodeGroupState ngs, UpdateQuorumRequest quorumUpdate) {
        if (ngs != null) {
            ...
            boolean isAvailable = NodeSelectorState.AVAILABLE.contains(NodeSelectorState.calculateStatus(getHost(), ngs));
            boolean isCurrentlyAvailable = currentState != null
                    && NodeSelectorState.AVAILABLE.contains(NodeSelectorState.calculateStatus(getHost(), currentState));
