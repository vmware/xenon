...
public class QueryTaskService extends StatefulService {
    ...
    public void handleGet(Operation get) {
        ...
        if (currentState.querySpec.options.contains(QueryOption.COUNT) && currentState.results
                .continuousResults != null) {
            System.out.println("Get on Query task from currentState " + currentState.results
                    .continuousResults.documentCountUpdated + "  " + r.continuousResults.documentCountUpdated);
            documentCount = currentState.results.documentCount;
            continuousResult = new ContinuousResult();
            continuousResult.documentCountAdded = currentState.results.continuousResults.documentCountAdded;
            continuousResult.documentCountUpdated = currentState.results.continuousResults.documentCountUpdated;
            continuousResult.documentCountDeleted = currentState.results.continuousResults.documentCountDeleted;
        }
        ...
        if (r.continuousResults != null) {
            currentState.results.documentCount = documentCount;
            currentState.results.continuousResults = continuousResult;
            System.out.println("Copied from local " + currentState.results
                        .continuousResults.documentCountUpdated);
        }
