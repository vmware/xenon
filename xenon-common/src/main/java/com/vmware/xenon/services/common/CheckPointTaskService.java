...
import static com.vmware.xenon.services.common.CheckPointTaskService.SubStage.GET_CHECK_POINTS;
import static com.vmware.xenon.services.common.CheckPointTaskService.SubStage.PATCH_CHECK_POINTS;
import static com.vmware.xenon.services.common.CheckPointTaskService.SubStage.SCAN_CHECK_POINT;
import static com.vmware.xenon.services.common.CheckPointTaskService.SubStage.SYNCHRONIZE;
...
public class CheckPointTaskService extends
    ...
    public static class State extends TaskService.TaskServiceState {
        ...
        /**
         * Self link of {@link NodeSelectorService} which place query tasks on selected nodes
         */
        @UsageOption(option = ServiceDocumentDescription.PropertyUsageOption.AUTO_MERGE_IF_NOT_NULL)
    ...
    public void handleStart(Operation startPost) {
        ...
        startPost.complete();
        sendSelfPatch(initialState, STARTED, subStageSetter(GET_CHECK_POINTS));
    }
    ...
    private void handleSubstage(State state) {
        switch (state.subStage) {
            case GET_CHECK_POINTS:
                handleGetCheckPoints(state, SCAN_CHECK_POINT);
                break;
            case SCAN_CHECK_POINT:
                handleScanCheckPoint(state, PATCH_CHECK_POINTS);
                break;
            case PATCH_CHECK_POINTS:
                handlePatchCheckPoints(state, SYNCHRONIZE);
                break;
            case SYNCHRONIZE:
    ...
    private void handleGetCheckPoints(State state, SubStage nextSubStage) {
        Operation get = Operation.createGet(UriUtils.buildUri(this.getHost(), state.checkPointServiceLink))
                ...
                .setCompletion((o, e) -> {
                    ...
                    sendSelfPatch(state, STARTED, subStageSetter(nextSubStage));
                });
    ...
    private boolean findNextCheckPoint(State state, Map<URI, Map<String, ServiceDocument>> results) {
        ...
        URI selectedUri = results.keySet().iterator().next();
        Map<String, ServiceDocument> selectedResult = results.get(selectedUri);
        Set<String> selectedLinks = results.get(selectedUri).keySet();
        Set<String> hitLinkSet = new HashSet<>();
        Map<String, ServiceDocument> missLatestVersion = new HashMap<>();
        // filter out hit link
        for (String link : selectedLinks) {
            boolean hit = true;
            for (URI u : results.keySet()) {
                if (u == selectedUri) {
                    // skip selected
                    continue;
                }
                Map<String, ServiceDocument> result = results.get(u);
                ServiceDocument doc = result.get(link);
                if (doc == null) {
                    hit = false;
                    break;
                }
                ServiceDocument selectedDoc = selectedResult.get(link);
            ...
            if (hit) {
                ServiceDocument hitDoc = selectedResult.get(link);
                // update hit time
        ...
        for (Map<String, ServiceDocument> result : results.values()) {
            for (String link : result.keySet()) {
                ...
                ServiceDocument missDoc = result.get(link);
                // filter out old version
                missLatestVersion.compute(link, (k, v) -> {
    ...
    protected boolean validateTransition(Operation patch, State currentTask,
        ...
        if (patchBody.taskInfo.stage == STARTED && patchBody.subStage == null) {
            patch.fail(new IllegalArgumentException("Missing substage"));
            return false;
        }
        if (currentTask.taskInfo != null && currentTask.taskInfo.stage != null) {
            if (currentTask.taskInfo.stage == STARTED
                    && patchBody.taskInfo.stage == STARTED) {
