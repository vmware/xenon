...
public class CheckPointTaskService extends
    ...
    public static class State extends TaskService.TaskServiceState {
        ...
        /**
         * Documents for on demand synchronization
         */
        @UsageOption(option = ServiceDocumentDescription.PropertyUsageOption.AUTO_MERGE_IF_NOT_NULL)
        public Map<String, ServiceDocument> missDocs;

        /**
         * number of documents scanned during query
         */
        @UsageOption(option = ServiceDocumentDescription.PropertyUsageOption.AUTO_MERGE_IF_NOT_NULL)
        public Integer totalDocsCount;
        ...
        Operation.createPost(forwardingService)
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (!validateSelectedNodes(state.selectedNodes, rsp.selectedNodes)) {
                        sendSelfCancellationPatch(state, String.format("selectedNodes miss match, expected %s, actual %s",
    ...
    private void handlePatchCheckPoints(State state, SubStage nextSubStage) {
        ...
        // logInfo("next check point %d", state.nextCheckPoint);

        AtomicInteger pending = new AtomicInteger(state.checkPointServiceUris.size());
        ...
        for (URI peerCheckPointServiceUri : state.checkPointServiceUris) {
    ...
    private void handleSynchronization(State state) {
        if (getHost().isStopping()) {
            sendSelfCancellationPatch(state, "host is stopping");
            return;
        }
        int total = state.missDocs.size();
        AtomicInteger pendingStarts = new AtomicInteger(total);
        Operation.CompletionHandler ch = (o, e) -> {
            // If synchronize individual service failed, next check point task should resolve it
            ...
            logInfo("Synchronized %d/%d services", state.synchCompletionCount, total);
            sendSelfFinishedPatch(state);
        };
        // log for debugging
        // logInfo("miss doc %s", Utils.toJsonHtml(state.missDocs));
        for (String link : state.missDocs.keySet()) {
            ...
            synchronizeService(link, ch);
        }
    }

    private void synchronizeService(String link, Operation.CompletionHandler c) {
        ...
        Operation synchRequest = Operation.createPost(this, factoryLink)
                .setBody(d)
                .setCompletion(c)
                .setReferer(getUri())
                .setConnectionSharing(true)
    ...
    private boolean findNextCheckPoint(State state, Map<URI, Map<String, ServiceDocument>> results) {
        ...
        // log for debugging
        // logInfo("hit link: %s", Utils.toJsonHtml(hitLinkSet));
        // collect miss
        ...
        state.totalDocsCount = hitLinkSet.size();
        hitLinkSet.clear();
        ...
        state.totalDocsCount += missDocs.size();
    ...
    private boolean validateInitialState(Operation startPost, State initialState) {
        ...
        if (initialState.taskLifetime != null) {
            initialState.documentExpirationTimeMicros = Utils.fromNowMicrosUtc(initialState.taskLifetime);
        }
        ...
        initialState.taskInfo.stage = TaskState.TaskStage.STARTED;
        initialState.subStage = SubStage.GET_CHECK_POINTS;
        initialState.synchCompletionCount = 0;
        initialState.start = Utils.getNowMicrosUtc();
        return true;
    }
