...
public class CheckPointTaskService extends
    ...
    private boolean findNextCheckPoint(State state, Map<URI, Map<String, ServiceDocument>> results) {
        ...
        Map.Entry<URI, Map<String, ServiceDocument>> selectedEntry = results.entrySet().iterator().next();
        URI selectedUri = selectedEntry.getKey();
        Map<String, ServiceDocument> selectedResult = selectedEntry.getValue();
        Set<String> selectedLinks = selectedResult.keySet();
