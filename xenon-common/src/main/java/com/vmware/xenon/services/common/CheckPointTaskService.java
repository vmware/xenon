...
public class CheckPointTaskService extends StatelessService {
    ...
    public static final long DEFAULT_CHECK_POINT_LAG_MILLISECONDS = 10;

    public static final int DEFAULT_WINDOW_SIZE_LIMIT = 100;
        ...
        Operation get = Operation.createGet(UriUtils.buildUri(this.getHost(), state.checkPointServiceLink))
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (!rsp.failures.isEmpty()) {
                        parent.fail(new IllegalStateException(String.format("get check points failed: %s", rsp.failures)));
                        return;
                    }
                    if (rsp.nodeCount < rsp.membershipQuorum) {
                        parent.fail(new IllegalStateException(String.format("response(%d) < membershipQuorum(%d)",
    ...
    private void scanCheckPoint(State state, Operation parent) {
        ...
        Operation.createPost(forwardingService)
                ...
                .setCompletion((o, e) -> {
                    ...
                    NodeGroupBroadcastResponse rsp = o.getBody((NodeGroupBroadcastResponse.class));
                    if (!rsp.failures.isEmpty()) {
                        parent.fail(new IllegalStateException(String.format("Query check points failed: %s", rsp.failures)));
                        return;
                    }
    ...
    private void patchCheckpoints(State state, Operation parent) {
